...
export default function FavoritesPage() {
    ...
    const removeFavorite = (id: number) => {
        ...
        setMessage('Produit supprimé des favoris.');
    };
    ...
    const handleBuy = (product: Product) => {
        ...
        setMessage(`Produit ${product.name} ajouté au panier !`);
    };
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {favorites.map((product) => (
                        <div key={product.id} className="rounded-lg p-4 shadow-md bg-bleuDiamant text-gray-900">
                            <img src={product.image} alt={product.name} className="w-full h-96 object-cover rounded mb-3" />
